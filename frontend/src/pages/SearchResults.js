...
const SearchResults = () => {
    ...
    const [team6Authors, setTeam6Authors] = useState(null);
    ...
    useEffect(() => {
        ...
        if (userID) {
            ...
            // team 6's remote URL
            const team6RemoteUrl = encodeURIComponent("https://cmput404-group6-instatonne.herokuapp.com/")
            createAPIEndpoint(`remote/authors/${team6RemoteUrl}`)
                .get()
                .then(res => {
                    setTeam6Authors(res.data.items
                        .filter(author => author["displayName"].toLowerCase().includes(query.toLowerCase())))
                })
                .catch(err => {
                    toast.error('An error has occurred.', {
                        description: 'Could not load team 6 authors at this time. Please try again later.',
                    });
                });
        ...
        <>
            <Layout>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <Typography variant="h6" align="left" sx={{ marginBottom: "10px" }}>Remote Team 6 Authors</Typography>
                        ...
                        <Card>
                            ...
                            {(team6Authors !== null && team6Authors.length === 0) &&
                                <>
                                    <PagesIcon sx={{ fontSize: "60px" }} />
                                    <Typography variant="h6" component="h2" sx={{ textAlign: "center" }}>No remote team 6 results.</Typography>
                                </>
                            }
